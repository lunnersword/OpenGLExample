...
class OpenGLShaderProgram {
    ...
    func addShaderFromFile(type: GLenum, file: String) -> Bool {
        if let source = OpenGLShaderProgram.shaderSourceFromFile(path: file), !source.isEmpty {
    ...
    func addShaderFromSource(type: GLenum, source: String) -> Bool {
        ...
        guard compiled == GL_TRUE else {
            OpenGLShaderProgram.checkShaderStatus(shader: shader)
            glDeleteShader(shader)
            shaders[type] = nil
            return false
        }
    ...
    func detachShader(type: GLenum) -> GLuint {
        if let oldShader = shaders[type] {
            glDetachShader(program, oldShader)
            return oldShader
        }
    ...
    func attachShader(type: GLenum, shader: GLuint) {
        guard type != GLenum(GL_NONE) else {
            return
        }
        guard shader > 0 else {
            return
    ...
    @discardableResult
    func link() -> Bool {
        return OpenGLShaderProgram.link(program: program)
    }
    ...
    func validate() {
        ...
        OpenGLShaderProgram.checkProgramStatus(program: program)
    }
...
extension OpenGLShaderProgram {
    ...
    class func addShaderFromFile(program: GLuint, vertexFile: String, fragmentFile: String) -> Bool {
        if let vertexSource = OpenGLShaderProgram.shaderSourceFromFile(path: vertexFile), let fragmentSource = OpenGLShaderProgram.shaderSourceFromFile(path: fragmentFile) {
            return OpenGLShaderProgram.addShadersFromSource(program:program, vertexSource:vertexSource, fragmentSource:fragmentSource)
        } else {
            #if DEBUG
    ...
    class func addShadersFromSource(program: GLuint, vertexSource: String, fragmentSource: String) -> Bool {
        ...
        OpenGLShaderProgram.link(program: program)
